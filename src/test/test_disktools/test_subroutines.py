...
from test.test_disktools import (
    SAMPLE_LIST_OF_BOGUS_PARAMS,
...
class TestIsThisADisk_ONE(unittest.TestCase):
    ...
    def testAbilityToNoticeDisks(self):
        for stub in SAMPLE_LIST_OF_DRIVES_STUBS:
            node = "/dev/%s" % (stub)
            self.assertTrue(
                is_this_a_disk(node, insist_on_this_existence_state=True),
                "%s *is* a disk, actually" % node,
            )
...
class TestPartitionNamedtupleFunction(unittest.TestCase):
    ...
    def testPPartUUID(self):
        ...
        self.assertEqual(
            partition.partuuid, partition_namedtuple(partition.node).partuuid
        )

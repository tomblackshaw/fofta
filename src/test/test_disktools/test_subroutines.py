...
def make_disk_have_one_randomizerd_partitionom_partition(disk_path, partno):
    ...
    add_partition(disk_path=disk_path, partno=partno,
                  start=start, end=end, fstype=fstype)
    ...
    def testAbilityToNoticeNotdisks(self):
        for stub in SAMPLE_LIST_OF_DRIVES_STUBS:
            ...
            for i in range(1, 105):
                node = '/dev/%s%d' % (stub, i)
            self.assertFalse(is_this_a_disk(
                node, insist_on_this_existence_state=True), "%s *isn't* a disk, actually" % node)
...
class TestIsThisADisk_TWO(unittest.TestCase):
    ...
    def testIfExistenceMakesADifference(self):
        for i in ('/dev/mmcblk0', '/dev/mmcblk1', '/dev/sda', '/dev/sdb', '/dev/sdp'):
            self.assertTrue(is_this_a_disk(
                device_path=i, insist_on_this_existence_state=True))
            self.assertTrue(is_this_a_disk(
                device_path=i, insist_on_this_existence_state=True))
...
#         self.assertEqual(should_be_str, but_is_str)
    ...
    def testPNode(self):
        (disk_path, start, end, fstype, pno,
         partition) = make_disk_have_one_randomizerd_partitionom_partition(MY_TESTDISK_PATH, 1)
        self.assertTrue(partition.node.startswith(MY_TESTDISK_PATH))
        self.assertTrue(partition.node.endswith(str(pno)))
        self.assertEqual(partition_namedtuple(
            partition.node).node, partition.node)

    def testPEnd(self):
        (disk_path, start, end, fstype, pno,
         partition) = make_disk_have_one_randomizerd_partitionom_partition(MY_TESTDISK_PATH, 1)
        self.assertEqual(
            end, start + partition_namedtuple(partition.node).size - 1)
    ...
    def testPUUID(self):
        (disk_path, start, end, fstype, pno,
         partition) = make_disk_have_one_randomizerd_partitionom_partition(MY_TESTDISK_PATH, 1)
        self.assertEqual(
            partition.uuid, partition_namedtuple(partition.node).uuid)

    def testPPartUUID(self):
        (disk_path, start, end, fstype, pno,
         partition) = make_disk_have_one_randomizerd_partitionom_partition(MY_TESTDISK_PATH, 1)
        self.assertEqual(partition.partuuid,
                         partition_namedtuple(partition.node).partuuid)

    def testPLabel(self):
        (disk_path, start, end, fstype, pno,
         partition) = make_disk_have_one_randomizerd_partitionom_partition(MY_TESTDISK_PATH, 1)
        self.assertEqual(
            partition.label, partition_namedtuple(partition.node).label)

    def testPPath(self):
        (disk_path, start, end, fstype, pno,
         partition) = make_disk_have_one_randomizerd_partitionom_partition(MY_TESTDISK_PATH, 1)
        self.assertEqual(
            partition.path, partition_namedtuple(partition.node).path)

    def testPType(self):
        (disk_path, start, end, fstype, pno,
         partition) = make_disk_have_one_randomizerd_partitionom_partition(MY_TESTDISK_PATH, 1)
        self.assertEqual(partition.fstype,
                         partition_namedtuple(partition.node).type)

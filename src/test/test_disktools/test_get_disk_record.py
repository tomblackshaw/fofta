...
class TestGetDiskRecord_TWO(unittest.TestCase):
    ...
    def testName(self):
        ...
        if r.partitiontable.id not in (None, ''):
            ...
            self.assertEqual(os.path.realpath(r.partitiontable.id),
        ...
        if r.partitiontable.node not in (None, ''):
            ...
            self.assertEqual(os.path.realpath(
                r.partitiontable.node), os.path.realpath(MY_TESTDISK_PATH))
...
class TestGetDiskRecord_TestFSTypeGetterAndSetter(unittest.TestCase):
    ...
    def testGetter(self):
        ...
        for fstype in ('83', '81', '83', '82', '80', '82', '83'):
            ...
            self.assertEqual(get_partition_fstype(
                d.node, d.partitions[0].partno), fstype)
    ...
    def testSetter(self):
        ...
        for old_fstype in ('83', '5', '83', '82', '5', '82', '83'):
            for new_fstype in ('83', '5', '83', '82', '5', '82', '83'):
                d.add_partition(partno=1, start=5555,
                                end=9999, fstype=old_fstype)
                self.assertEqual(get_partition_fstype(
                    d.node, d.partitions[0].partno), old_fstype)
                set_partition_fstype(
                    d.node, d.partitions[0].partno, new_fstype)
                self.assertEqual(get_partition_fstype(
                    d.node, d.partitions[0].partno), new_fstype)
